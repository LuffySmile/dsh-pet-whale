...
/* 全屏海洋：执行任务时铺满屏幕；pointer-events:none 点击穿透，不挡操作 */
.dshpet-ocean {
  position: fixed;
  inset: 0;
  z-index: 1;
...
.dshpet-fish-b { width: 20px; animation-duration: 34s; }
.dshpet-fish-c { width: 24px; animation-duration: 40s; animation-direction: reverse; }
.dshpet-fish-c .dshpet-fish-flip { transform: scaleX(-1); }
...
/* 海底：小虾（原地小步挪动） */
.dshpet-shrimp { position: absolute; width: 30px; animation: dshpet-shrimp-scuttle 16s ease-in-out infinite; }
...
`;
		...
		function Pet() {
			...
			React.useEffect(() => {
				...
				const pickTarget = () => {
					...
					const maxY = Math.max(m + 60, vh - 90);
					rs.tx = m + Math.random() * (maxX - m);
					rs.ty = m + Math.random() * (maxY - m);
				};
			...
			const ocean = React.createElement('div', { className: 'dshpet-ocean' + (active ? ' dshpet-ocean-on' : '') },
				...
				React.createElement('div', { className: 'dshpet-ocean-floor' },
					...
					React.createElement('div', { className: 'dshpet-fish', style: { width: '28px', bottom: '16%' } },
						React.createElement('div', { className: 'dshpet-fish-flip' }, fishSvg('#ffb86b')),
					),
					React.createElement('div', { className: 'dshpet-fish dshpet-fish-b', style: { bottom: '8%' } },
						React.createElement('div', { className: 'dshpet-fish-flip' }, fishSvg('#ffe08a')),
					),
					React.createElement('div', { className: 'dshpet-fish dshpet-fish-c', style: { bottom: '21%' } },
						React.createElement('div', { className: 'dshpet-fish-flip' }, fishSvg('#7fdbda')),
					),
					React.createElement('div', { className: 'dshpet-shrimp', style: { left: '14%', bottom: '2%' } }, shrimpSvg('#ff9e6d')),
					React.createElement('div', { className: 'dshpet-shrimp', style: { left: '60%', bottom: '2%', animationDelay: '2.4s' } }, shrimpSvg('#f4a7b9')),
				),
			);
		...
		// 全屏海洋气泡分布（左%、尺寸px、时长s、延迟s）
		const OCEAN_BUBBLES = [
			{ left: '4%', size: 14, dur: 9, delay: 0 },
			{ left: '10%', size: 7, dur: 12, delay: 3.1 },
			{ left: '16%', size: 9, dur: 11, delay: 2.4 },
			{ left: '23%', size: 6, dur: 13, delay: 5.6 },
			{ left: '30%', size: 16, dur: 10, delay: 5.1 },
			{ left: '38%', size: 8, dur: 11.5, delay: 7.2 },
			{ left: '46%', size: 8, dur: 12, delay: 1.2 },
			{ left: '52%', size: 12, dur: 9.8, delay: 3.6 },
			{ left: '57%', size: 13, dur: 9.5, delay: 3.8 },
			{ left: '63%', size: 7, dur: 12.5, delay: 6.8 },
			{ left: '70%', size: 10, dur: 11.5, delay: 6.3 },
			{ left: '76%', size: 15, dur: 10.5, delay: 0.8 },
			{ left: '82%', size: 6, dur: 13.5, delay: 4.9 },
			{ left: '87%', size: 12, dur: 10.2, delay: 2.1 },
			{ left: '93%', size: 9, dur: 12.5, delay: 4.6 },
		];
