...
.dshpet-jet { fill: rgba(190,245,235,0.92); transform-origin: 50% 100%; animation: dshpet-jet 0.9s ease-in-out infinite; }
.dshpet-drop { fill: rgba(190,245,235,0.95); animation: dshpet-drop 0.9s ease-out infinite; }
...
@keyframes dshpet-jet {
  0%, 100% { transform: scaleY(0.55); opacity: 0.8; }
...
.dshpet-btn {
  ...
  background: rgba(14,142,166,0.92);
  border-radius: 7px; padding: 3px 9px; font-size: 11px;
  font-family: inherit;
}
.dshpet-btn:hover { background: rgba(10,118,140,1); }
...
.dshpet-restore {
  ...
  background: rgba(14,142,166,0.92); color: #fff; font-size: 15px;
...
.dshpet-switch input:checked + .dshpet-switch-slider { background: #0e8ea6; }
...
`;
		...
		// 启用/停用偏好：localStorage 持久化，设置页开关可随时切换
		...
		const petPref = {
			...
			read() {
				...
				} catch (_) { this.enabled = true; }
			},
			set(enabled) {
				this.enabled = enabled;
				try { localStorage.setItem(PET_LS_KEY, enabled ? 'true' : 'false'); } catch (_) {}
				for (const fn of [...this.listeners]) { try { fn(enabled); } catch (_) {} }
			},
			subscribe(fn) {
		...
		function Pet() {
			...
			const [petEnabled, setPetEnabled] = React.useState(petPref.enabled);

			React.useEffect(() => petPref.subscribe(setPetEnabled), []);
			...
			const whale = React.createElement('div', {
				className: rootClass,
				style: pos && !roaming ? { left: pos.x + 'px', top: pos.y + 'px' } : null,
						...
						React.createElement('svg', { className: 'dshpet-svg', viewBox: '0 0 50 50' },
							React.createElement('defs', null,
								React.createElement('linearGradient', { id: 'dshpet-grad', x1: '0', y1: '0', x2: '1', y2: '1' },
									React.createElement('stop', { offset: '0%', stopColor: '#33d6c4' }),
									React.createElement('stop', { offset: '100%', stopColor: '#0e8ea6' }),
								),
							),
							...
							React.createElement('g', { className: 'dshpet-whale dshpet-whale-' + state },
								...
								state === 'spraying' ? React.createElement('g', null,
									React.createElement('path', { className: 'dshpet-jet', d: 'M39.4 14 C39.2 11.6 39.8 9.6 40.5 8 C41.2 9.6 41.8 11.6 41.6 14 Z' }),
									React.createElement('circle', { className: 'dshpet-drop', cx: '38.2', cy: '8.6', r: '0.5', style: { animationDelay: '0.12s' } }),
									React.createElement('circle', { className: 'dshpet-drop', cx: '40.5', cy: '7.2', r: '0.55', style: { animationDelay: '0.42s' } }),
									React.createElement('circle', { className: 'dshpet-drop', cx: '42.8', cy: '8.6', r: '0.5', style: { animationDelay: '0.72s' } }),
								) : null,
							),
		...
		function PetSettings() {
			const [enabled, setEnabled] = React.useState(petPref.enabled);
			React.useEffect(() => petPref.subscribe(setEnabled), []);

			return React.createElement('div', { className: 'dshpet-settings' },
				React.createElement('div', { className: 'dshpet-settings-row' },
					...
					React.createElement('label', { className: 'dshpet-switch' },
						React.createElement('input', {
							type: 'checkbox',
							checked: enabled,
							onChange: (e) => { e.stopPropagation(); petPref.set(e.target.checked); },
						}),
