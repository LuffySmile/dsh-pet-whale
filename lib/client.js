...
/* 全屏海洋：执行任务时覆盖屏幕下 3/4；pointer-events:none 点击穿透，不挡操作 */
.dshpet-ocean {
  position: fixed;
  left: 0;
  right: 0;
...
.dshpet-ocean-wave {
  ...
  width: 200%;
  height: 18px;
  animation: dshpet-wave-scroll 14s linear infinite;
}
.dshpet-ocean-wave-2 { top: 5px; animation-duration: 20s; animation-direction: reverse; }
@keyframes dshpet-wave-scroll {
  0% { transform: translateX(0); }
  100% { transform: translateX(-50%); }
}

/* 海面破浪：泡沫浪花带 + 卷浪（独立层，可溢出到海面线以上） */
.dshpet-ocean-foamlayer {
  position: fixed;
  left: 0;
  right: 0;
  top: calc(25% - 32px);
...
.dshpet-ocean-foamlayer.on { opacity: 1; }
.dshpet-ocean-foam {
  position: absolute;
  top: 32px;
  left: 0;
  ...
  height: 7px;
  border-radius: 3px;
  background: repeating-linear-gradient(90deg, rgba(255,255,255,0.5) 0 22px, rgba(255,255,255,0) 22px 44px);
  animation: dshpet-wave-scroll 16s linear infinite;
}
.dshpet-break {
  position: absolute;
  top: 10px;
  animation: dshpet-break 9s ease-in-out infinite;
}
.dshpet-break svg { width: 100%; height: auto; display: block; }
@keyframes dshpet-break {
...
/* 海底：小虾（沿海底巡逻游动） */
.dshpet-shrimp { position: absolute; animation: dshpet-shrimp-swim 18s ease-in-out infinite; }
.dshpet-shrimp svg { width: 100%; height: auto; display: block; }
@keyframes dshpet-shrimp-swim {
  0%, 100% { transform: translateX(0) translateY(0); }
  14% { transform: translateX(22px) translateY(-2px); }
  28% { transform: translateX(44px) translateY(0); }
  42% { transform: translateX(66px) translateY(-2px); }
  52% { transform: translateX(66px) translateY(-1px); }
  68% { transform: translateX(42px) translateY(0); }
  84% { transform: translateX(20px) translateY(-2px); }
}
...
`;
		...
		function Pet() {
			...
			React.useEffect(() => {
				...
				const pickTarget = () => {
					...
					const minY = Math.max(m, Math.round(vh * 0.26));
			...
			// 海面破浪层：泡沫带 + 卷浪（独立 fixed 层，可越过海面线）
			const foamLayer = React.createElement('div', { className: 'dshpet-ocean-foamlayer' + (active ? ' on' : '') },
				React.createElement('div', { className: 'dshpet-ocean-foam' }),
				BREAK_SPECS.map((w, i) =>
					React.createElement('div', {
						key: 'w' + i,
						className: 'dshpet-break',
						style: { left: w.left, width: w.width + 'px', animationDelay: w.delay + 's' },
					}, breakWaveSvg()),
				),
			);
		...
		function breakWaveSvg() {
			return React.createElement('svg', { viewBox: '0 0 44 18' },
				React.createElement('path', { d: 'M4 17 C4 11 10 7 20 6.5 C26 6.2 30 7.2 33 9 C30.5 6 25 4.5 19 5 C13 5.5 8 8.5 7 13 C6.5 15 5 17 4 17 Z', fill: 'rgba(140,215,235,0.9)' }),
				React.createElement('path', { d: 'M33 9 C35 7 39 5.5 42 6 C44.5 6.4 45.5 8 44.5 9.5 C42 8.5 38 8.8 34.5 10.5 Z', fill: 'rgba(140,215,235,0.9)' }),
				React.createElement('path', { d: 'M33 9 C33.8 7.2 36 6 38.5 6.3 C40.8 6.6 41.8 8 41 9.2 C39.5 8.2 36.5 8.4 34.5 9.6 Z', fill: '#ffffff', opacity: '0.95' }),
				React.createElement('circle', { cx: '36', cy: '5.6', r: '0.9', fill: '#ffffff' }),
				React.createElement('circle', { cx: '39.5', cy: '5.4', r: '0.7', fill: '#ffffff' }),
				React.createElement('circle', { cx: '41.5', cy: '7', r: '0.6', fill: '#ffffff' }),
				React.createElement('circle', { cx: '31', cy: '7.6', r: '0.7', fill: '#ffffff', opacity: '0.85' }),
				React.createElement('circle', { cx: '33.5', cy: '6.4', r: '0.5', fill: '#ffffff', opacity: '0.8' }),
			);
		}
		...
		const BREAK_SPECS = [
			{ left: '6%', width: 36, delay: 0 },
			{ left: '28%', width: 26, delay: 2.4 },
			{ left: '50%', width: 42, delay: 4.6 },
			{ left: '72%', width: 28, delay: 1.3 },
			{ left: '90%', width: 34, delay: 3.5 },
		];
